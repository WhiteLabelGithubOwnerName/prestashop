...
use WhiteLabelMachineName\Sdk\Model\AbstractTransactionPending;
...
class WhiteLabelMachineNameServiceTransaction extends WhiteLabelMachineNameServiceAbstract
{
    ...
    private const POSSIBLE_PAYMENT_METHOD_SESSION_KEY = 'wlmPossiblePaymentMethods';
     ...
     *
     * @var \WhiteLabelMachineName\Sdk\Service\TransactionLineItemVersionService
     */
     ...
     *
     * @var \WhiteLabelMachineName\Sdk\Model\ChargeAttempt[]
     */
    ...
    {
        if ($property === null) {
            $property = new $className(WhiteLabelMachineNameHelper::getApiClient());
        }
        return $property;
    }
    ...
    protected function getTransactionService()
    {
        return $this->getSdkService(
            ...
            \WhiteLabelMachineName\Sdk\Service\TransactionService::class
        );
    }
    ...
    protected function getTransactionIframeService()
    {
        return $this->getSdkService(
            ...
            \WhiteLabelMachineName\Sdk\Service\TransactionIframeService::class
        );
    }
    ...
    protected function getTransactionPaymentPageService()
    {
        return $this->getSdkService(
            ...
            \WhiteLabelMachineName\Sdk\Service\TransactionPaymentPageService::class
        );
    }
    ...
    protected function getChargeAttemptService()
    {
        return $this->getSdkService(
            ...
            \WhiteLabelMachineName\Sdk\Service\ChargeAttemptService::class
        );
    }
    ...
    public function getJavascriptUrl(Cart $cart)
    {
        ...
        $url = $js . '&className=whitelabelmachinenameIFrameCheckoutHandler';
     ...
     * @param int $spaceId
     * @param int $transactionId
     * @return \WhiteLabelMachineName\Sdk\Model\ChargeAttempt|null
     */
    ...
    public function updateTransactionInfo(\WhiteLabelMachineName\Sdk\Model\Transaction $transaction, Order $order)
    {
        ...
        if (
            $transaction->getState() == \WhiteLabelMachineName\Sdk\Model\TransactionState::FAILED
        ...
        if (!isset($this->chargeAttemptCache[$key])) {
            ...
            $filter->setChildren(
                ...
                $this->createEntityFilter(
                    'state',
                    \WhiteLabelMachineName\Sdk\Model\ChargeAttemptState::SUCCESSFUL
                ),
                )
            );
            $query->setFilter($filter);
    ...
    public function getPossiblePaymentMethods(
        Cart $cart,
        \WhiteLabelMachineName\Sdk\Model\Transaction $transaction = null
    ) {
     ...
     *
     * @param Cart $cart
     * @param \WhiteLabelMachineName\Sdk\Model\Transaction|null $transaction
    ...
    private function warmPossiblePaymentMethodCache(
        ...
        $cached = WhiteLabelMachineNameHelper::getCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
        ...
        try {
            ...
            WhiteLabelMachineNameHelper::clearCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
            $this->clearSessionPaymentMethodCacheEntry($currentCartId);
            if ($failSilently) {
                return array();
            }
    ...
    private function getSessionPaymentMethodCacheData()
    {
        ...
        $decoded = WhiteLabelMachineNameTools::base64Decode($cookie->$key);
    ...
    private function persistSessionPaymentMethodCacheData(array $data)
    {
        ...
        if (empty($data)) {
            ...
            $cookie->$key = WhiteLabelMachineNameTools::base64Encode(serialize($data));
        }
        $cookie->write();
    }
    ...
    private function convertPaymentMethodsForSession(array $paymentMethods)
    {
        ...
        foreach ($paymentMethods as $method) {
            if ($method instanceof \WhiteLabelMachineName\Sdk\Model\PaymentMethodConfiguration) {
    ...
    {
        $method = new \WhiteLabelMachineName\Sdk\Model\PaymentMethodConfiguration();
    ...
    public function invalidateTransactionCache(Cart $cart)
    {
        ...
        WhiteLabelMachineNameHelper::clearCartMeta($cart, self::CART_HASH_META_KEY);
        WhiteLabelMachineNameHelper::clearCartMeta($cart, self::POSSIBLE_PAYMENT_METHOD_CACHE_KEY);
     ...
     * @param int $spaceId
     * @param int $transactionId
     * @return \WhiteLabelMachineName\Sdk\Model\Transaction|null
     */
    private function getCachedTransactionForCart(Cart $cart, $spaceId, $transactionId)
    {
        $cached = WhiteLabelMachineNameHelper::getCartMeta($cart, self::TRANSACTION_CACHE_META_KEY);
        if (!is_array($cached)
        ...
        $serialized = WhiteLabelMachineNameTools::base64Decode($cached['data']);
        $transaction = @unserialize($serialized);
        if ($transaction instanceof \WhiteLabelMachineName\Sdk\Model\Transaction) {
     ...
     * @return void
     */
    private function storeCachedTransactionForCart(Cart $cart, \WhiteLabelMachineName\Sdk\Model\Transaction $transaction)
    {
        ...
        WhiteLabelMachineNameHelper::updateCartMeta(
            ...
            array(
                ...
                'data' => WhiteLabelMachineNameTools::base64Encode(serialize($transaction))
            )
        );
    }
    ...
    {
        WhiteLabelMachineNameHelper::clearCartMeta($cart, self::TRANSACTION_CACHE_META_KEY);
    }
     ...
     * @return void
     */
    private function cacheLoadedTransactionObject(\WhiteLabelMachineName\Sdk\Model\Transaction $transaction)
    {
        if ($transaction == null) {
            return;
        }
     ...
     * @return string|null
     */
    private function getCachedJavascriptUrl(Cart $cart, \WhiteLabelMachineName\Sdk\Model\Transaction $transaction)
    {
        $cached = WhiteLabelMachineNameHelper::getCartMeta($cart, self::JS_URL_CACHE_META_KEY);
        if (!is_array($cached)
    ...
    private function storeCachedJavascriptUrl(
        Cart $cart,
        \WhiteLabelMachineName\Sdk\Model\Transaction $transaction,
        $url
    ) {
        WhiteLabelMachineNameHelper::updateCartMeta(
            $cart,
    ...
    {
        WhiteLabelMachineNameHelper::clearCartMeta($cart, self::JS_URL_CACHE_META_KEY);
    }
    ...
    {
        $hash = WhiteLabelMachineNameHelper::getCartMeta($cart, self::CART_HASH_META_KEY);
    ...
    {
        WhiteLabelMachineNameHelper::updateCartMeta($cart, self::CART_HASH_META_KEY, $cartHash);
    }
    ...
    public function refreshTransactionFromCart(Cart $cart)
    {
        ...
        if ($transaction === null
            || $transaction->getState() !== \WhiteLabelMachineName\Sdk\Model\TransactionState::PENDING
        ) {
            return null;
        }
    ...
    protected function createTransactionFromCart(Cart $cart)
    {
        ...
        $createTransaction->setCustomersPresence(
            \WhiteLabelMachineName\Sdk\Model\CustomersPresence::VIRTUAL_PRESENT
        );
        ...
        $spaceViewId = Configuration::get(
            WhiteLabelMachineNameBasemodule::CK_SPACE_VIEW_ID,
            null,
            null,
            $cart->id_shop
        );
        ...
        $this->storeTransactionCartHash($cart, WhiteLabelMachineNameHelper::calculateCartHash($cart));
        $this->clearCachedJavascriptUrl($cart);
    ...
    protected function loadAndUpdateTransactionFromCart(Cart $cart)
    {
        ...
        $transaction = $this->getTransaction($ids['spaceId'], $ids['transactionId']);
        if ($transaction === null || $transaction->getState() !== \WhiteLabelMachineName\Sdk\Model\TransactionState::PENDING) {
            $transaction = $this->createTransactionFromCart($cart);
        }

        $customerId   = $transaction->getCustomerId();
        $cartCurrency = WhiteLabelMachineNameHelper::convertCurrencyIdToCode($cart->id_currency);
        $cartHash     = WhiteLabelMachineNameHelper::calculateCartHash($cart);
    ...
    protected function updateTransactionFromCart(
        ...
        $cartHash = $cartHash ?: WhiteLabelMachineNameHelper::calculateCartHash($cart);
