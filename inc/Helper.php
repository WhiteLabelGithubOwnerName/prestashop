    ...
    */
    ...
    public static function storeDeferredEmails($recordedMessages, Order $order)
    {
        ...
        if (count($deferredMessages) > 0) {
            self::storeOrderEmails($order, $deferredMessages, WhiteLabelMachineNameBasemodule::EMAIL_KEY_DOWNLOAD);
        }
    }
    ...
    {
        if (!Configuration::get(WhiteLabelMachineNameBasemodule::CK_MAIL, null, null, $order->id_shop)) {
            self::deleteOrderEmails($order, WhiteLabelMachineNameBasemodule::EMAIL_KEY_DOWNLOAD);
            return;
        }
        $deferredMessages = self::getOrderEmails($order, WhiteLabelMachineNameBasemodule::EMAIL_KEY_DOWNLOAD);
