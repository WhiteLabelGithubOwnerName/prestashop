...
class PaymentTerminalService {
	 ...
	 * @throws \WhiteLabelMachineName\Sdk\VersioningException
	 * @throws \WhiteLabelMachineName\Sdk\Http\ConnectionException
	 * @return \WhiteLabelMachineName\Sdk\Model\PaymentTerminalTransactionSummaryReference
	 */
	...
	public function triggerFinalBalanceWithHttpInfo($space_id, $terminal_id) {
		...
		try {
			$response = $this->apiClient->callApi(
				...
				$httpBody,
				$headerParams,
				'\WhiteLabelMachineName\Sdk\Model\PaymentTerminalTransactionSummaryReference',
				'/payment-terminal/trigger-final-balance'
            );
			return new ApiResponse($response->getStatusCode(), $response->getHeaders(), $this->apiClient->getSerializer()->deserialize($response->getData(), '\WhiteLabelMachineName\Sdk\Model\PaymentTerminalTransactionSummaryReference', $response->getHeaders()));
		} catch (ApiException $e) {
			switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
